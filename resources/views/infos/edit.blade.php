    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="mb-8">
            <div class="flex items-center justify-between">
                <div>
                    <h1 class="text-3xl font-bold text-gray-900">Modifier l'info</h1>
                    <p class="mt-1 text-sm text-gray-600">Modifiez les informations de l'info et prévisualisez les changements</p>
                </div>
        ...
        <div class="flex flex-col lg:flex-row gap-8">
            ...
            <div class="flex-1">
                <div class="bg-white rounded-lg shadow-lg p-6">
                    <form action="{{ route('infos.update', $info->id) }}" method="POST" enctype="multipart/form-data" id="infoForm">
                        @csrf
                        @method('PUT')
                            ...
                            <select id="category_id" 
                                ...
                                @foreach($categories as $category)
                                    <option value="{{ $category->id }}" 
                                            {{ old('category_id', $info->category_id) == $category->id ? 'selected' : '' }}
                        ...
                        <div class="mb-6">
                            ...
                            <input type="url" 
                                   id="external_link" 
                                   name="external_link"
                                   value="{{ old('external_link', $info->external_link) }}"
                                   class="form-input w-full rounded-lg border-gray-300 focus:border-blue-500 focus:ring focus:ring-blue-200">
                        </div>
                    ...
                    <div class="p-6">
                        <div id="preview" class="prose max-w-none">
                            ...
                            <div id="previewImageContainer" class="relative mb-4 rounded-lg overflow-hidden">
                                <img id="previewImage" 
                                     src="{{ $info->image ? asset('storage/' . $info->image) : '' }}" 
                                     alt=""
                            ...
                            <!-- Lien externe -->
                            <div id="previewLinkContainer" class="mt-4 {{ $info->external_link ? '' : 'hidden' }}">
                                <a id="previewLink" 
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    titleInput.addEventListener('input', function() {
        previewTitle.textContent = this.value || 'Titre de l\'info';
    });

    // Mise à jour du contenu
    contentInput.addEventListener('input', function() {
        previewContent.textContent = this.value || 'Contenu de l\'info';
    });
